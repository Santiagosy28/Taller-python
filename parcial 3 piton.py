...
class LoginWindow:
    ...
    def setup_ui(self):
        ...
        self.login_button = tk.Button(self.master, text="Iniciar sesión", command=lambda: self.iniciar_sesion())
        self.login_button.pack(pady=20)

    def iniciar_sesion(self):
    
        usuario = lambda: self.usuario_entry.get().strip()
        clave = lambda: self.clave_entry.get().strip()
        ...
        try:
            ...
            usuario_valido = next((u for u in lista_usuarios if u["usuario"] == usuario() and u["clave"] == clave()), None)
